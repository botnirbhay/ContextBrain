...
function ensureAgentBridge(root, { force }) {
  ...
  const content = [
    ...
    "",
    "This repository uses CodexMemory for persistent local project memory.",
    "",
    "When working in this repo:",
    "",
    "1. Before coding, inspect `.codexmemory/AGENTS.md` if present.",
    "2. Retrieve task-specific memory with `codexmemory context \"<task>\"` when the command is available.",
    "3. Prefer current repository code over memory if they conflict, and surface the conflict.",
    "4. After work, record durable outcomes with `codexmemory session note`, `codexmemory session add-file`, and `codexmemory session stop --summary \"...\"` when a CodexMemory session is active.",
    "5. Do not store raw chat history as memory; keep only decisions, conventions, bug fixes, failed attempts, lessons, warnings, and todos.",
    ""
  ].join("\n");
