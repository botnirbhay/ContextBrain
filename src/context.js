...
export function formatContextPack(pack, { includeTask = true } = {}) {
  ...
  if (pack.groups.length === 0) {
    lines.push("No relevant CodexMemory entries found.");
    return lines.join("\n");
  }
...
export function buildCodexPrompt(task, { root = process.cwd(), markUsed = false } = {}) {
  ...
  return {
    ...
    prompt: [
      ...
      "After completing the work, summarize durable decisions, failed attempts, bug fixes, changed files, commands run, and final outcome so CodexMemory can reflect on them."
    ].join("\n")
  };
}
