...
test("package exposes only the codemem binary and production files", () => {
  const pkg = JSON.parse(fs.readFileSync("package.json", "utf8"));

  assert.deepEqual(Object.keys(pkg.bin), ["codemem"]);
  assert.equal(pkg.bin.codemem, "./src/codemem.js");
